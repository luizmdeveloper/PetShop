...
public class FrmNovoSchedule extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public FrmNovoSchedule(FrmCadastroSchedule control) {
		this.control = control;
		setLocationRelativeTo(null);
		setTitle("Projeto SIGMA - Novo Schedule");
		setIconImage(Toolkit.getDefaultToolkit().getImage(FrmNovoSchedule.class.getResource("/res/logo.png")));
		setResizable(false);
		...
		btnNovo.setIcon(new ImageIcon(FrmNovoSchedule.class.getResource("/res/incluir.png")));
		...
		btnCancelar.setIcon(new ImageIcon(FrmNovoSchedule.class.getResource("/res/Sair.png")));
	...
	protected void novoScheduleOnClick() {
		Schedule schedule = new Schedule();
		Animal animal = new Animal();
		animal.setId(animais.get(cmbAnimal.getSelectedIndex()).getId());
		schedule.setData(converterData(txtData.getText()));
		schedule.setIdanimal(animal);
		schedule.setType((ScheduleType) cmbTipo.getSelectedItem());
	...
	private void persistir(Schedule schedule) {
		...
		try {
			scheduleDAO.inserir(schedule);
			JOptionPane.showMessageDialog(this, "Schedule, foi inserido com sucesso !!");
			limparCampos();
			this.control.refreshShcedule();
		} catch (PersistenciaException e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(this, "Erro na inserção do Schedule \n"+e.getMessage(),"Projeto SIGMA LTDA", JOptionPane.ERROR_MESSAGE);
		}
	...
	protected void cancelarOnClick() {
		...
		carregarTelaDeCadastro();
	}

	private void carregarTelaDeCadastro() {
