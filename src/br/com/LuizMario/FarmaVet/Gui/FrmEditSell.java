...
public class FrmEditSell extends JDialog {
	...
	public FrmEditSell(Sell sell) {
		...
		btnAdicionarItens.setIcon(new ImageIcon(FrmEditSell.class.getResource("/res/produto.png")));
		...
		btnDeletar.setIcon(new ImageIcon(FrmEditSell.class.getResource("/res/Sair.png")));
		...
		btnSalvar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				alterarNovoSellOnClick();
			}
		});
		btnSalvar.setIcon(new ImageIcon(FrmEditSell.class.getResource("/res/incluir.png")));
		...
		btnCancelar.setIcon(new ImageIcon(FrmEditSell.class.getResource("/res/Sair.png")));
		...
		iniciarCampos(this.sell);
	}

	protected void alterarNovoSellOnClick() {
	...
	private void persistir(Sell sell) {
		...
		try {
			sellDAO.delete(this.sell.getId());
	...
	private void iniciarCampos(Sell sell) {
		SimpleDateFormat format = new SimpleDateFormat("dd/MM/yyyy");
		String data = format.format(sell.getData());
		txtData.setText(data);
		lbTotal.setText("R$ "+sell.getTotal());
		iniciarComboVendedorSelecionado(sell.getVendedor().getId());
		sell.setItens(itensDaVenda(sell.getId()));
		refreshItens(sell.getItens());
	}

	private ArrayList<SellItem> itensDaVenda(int id) {
		SellItemDAO sellItemDAO = new SellItemDAO();
		ArrayList<SellItem> itensDaVenda = new ArrayList<SellItem>();
		try {
			itensDaVenda = sellItemDAO.buscarItens(id);
		} catch (PersistenciaException e) {
			e.printStackTrace();
			JOptionPane.showMessageDialog(this, "Erro ao carregar os itens da venda\n"+e.getMessage(),"Projeto SIGMA LTDA",JOptionPane.ERROR_MESSAGE);
		}
		return itensDaVenda;
	}

	private void iniciarComboVendedorSelecionado(int id) {
		int posicao = 0;
		...
		cmbVendedor.setSelectedIndex(posicao);
	}
